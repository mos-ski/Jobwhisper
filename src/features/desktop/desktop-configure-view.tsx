...
export function DesktopConfigureView() {
  ...
  function handleContinue() {
    if (step === 1) {
      ...
      navigate('/desktop/session')
    }
  }

  return (
    <div className="flex h-full min-h-[520px] items-center justify-center bg-live-workspace px-6 py-12">
      <div className="flex max-h-[640px] w-full max-w-[485px] flex-col overflow-hidden rounded-lg bg-[#0d1929] shadow-panel">
        <div className="grid shrink-0 gap-2 border-b border-white/15 py-6 text-center">
          <h1 className="text-xl font-medium text-white">Configure your interview</h1>
          <div className="flex items-center justify-center gap-1.5">
            <span className={cn('h-1.5 w-6 rounded-full transition-colors', step === 1 ? 'bg-[#0052ff]' : 'bg-white/15')} />
            <span className={cn('h-1.5 w-6 rounded-full transition-colors', step === 2 ? 'bg-[#0052ff]' : 'bg-white/15')} />
          </div>
        </div>

        <div className="flex shrink-0 items-center justify-between bg-[rgba(221,241,255,0.13)] px-4 py-1.5">
        ...
        <div className="flex shrink-0 items-center justify-between border-t border-white/15 px-6 py-4">
          <button type="button" onClick={handleBack} className="inline-flex items-center gap-1 text-base font-semibold text-white">
