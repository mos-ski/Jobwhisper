import { FONT, brand, button, divider, escapeHtml, greeting, heading, paragraph, pill, renderEmailShell } from '../shell'
...
function jobRow(job: (typeof JOBS)[number], isLast: boolean): string {
  return `<tr>
<td style="padding:16px 0;${isLast ? '' : `border-bottom:1px solid ${brand.line};`}">
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
<tr>
<td>
<p style="margin:0 0 3px;font-family:${FONT};font-size:15px;font-weight:600;color:${brand.ink};">${escapeHtml(job.title)}</p>
<p style="margin:0;font-family:${FONT};font-size:13px;color:${brand.muted};">${escapeHtml(job.company)} &middot; ${escapeHtml(job.location)}</p>
</td>
...
function jobList(): string {
  ...
  return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:4px 0 20px;background-color:${brand.white};border:1px solid ${brand.line};border-radius:10px;">
...
${divider()}
${paragraph("You're getting this because job alerts are turned on for your account.", { muted: true })}
