import { FONT, brand, button, divider, escapeHtml, greeting, heading, paragraph, renderEmailShell } from '../shell'
...
function actionItemRow(text: string, isLast: boolean): string {
  return `<tr>
<td style="padding:12px 0;vertical-align:top;width:20px;${isLast ? '' : `border-bottom:1px solid ${brand.line};`}">
<span style="display:inline-block;width:6px;height:6px;margin-top:8px;border-radius:50%;background-color:${brand.ink};"></span>
</td>
<td style="padding:12px 0;font-family:${FONT};font-size:14px;line-height:21px;color:${brand.ink};${isLast ? '' : `border-bottom:1px solid ${brand.line};`}">${escapeHtml(text)}</td>
</tr>`
}

function actionItemList(): string {
  const rows = ACTION_ITEMS.map((item, i) => actionItemRow(item, i === ACTION_ITEMS.length - 1)).join('')
  return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 20px;background-color:${brand.white};border:1px solid ${brand.line};border-radius:10px;">
...
<p style="margin:0 0 8px;font-family:${FONT};font-size:13px;font-weight:600;text-transform:uppercase;letter-spacing:0.03em;color:${brand.muted};">Summary</p>
...
<p style="margin:0 0 8px;font-family:${FONT};font-size:13px;font-weight:600;text-transform:uppercase;letter-spacing:0.03em;color:${brand.muted};">Action items</p>
${actionItemList()}
${button('#', 'View full transcript')}
${divider()}
${paragraph('Recaps are generated automatically. Correct anything that looks off directly in the app.', { muted: true })}
