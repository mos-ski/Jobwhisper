import { brand, button, divider, escapeHtml, heading, paragraph, renderEmailShell } from '../shell'
...
function actionItemRow(text: string): string {
  return `<tr>
...
<td style="padding:0 0 10px;font-family:${FONT};font-size:14px;line-height:21px;color:${brand.textLight};">${escapeHtml(text)}</td>
</tr>`
}
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 20px;">${ACTION_ITEMS.map(actionItemRow).join('')}</table>
