...
**Checkout flow needs rethinking, not just re-pricing.** The order-bump description below (add Resume Builder/Auto Apply to a cart at subscribe time, for a flat recurring price) was written for the flat-fee model and doesn't translate cleanly to pure usage billing — you can't "add $1-per-job Auto Apply to your cart" the same way. **Decided 2026-09-02: Auto Apply is removed from the order bump entirely** — it's not offered as a signup-time add-on anymore. Resume Builder's fate in the order bump is a separate, still-open question (`PRICING.md` §6.7) — don't assume it stays or goes without checking. Whatever remains of the order bump (if anything) probably becomes a feature toggle at signup rather than a priced cart line item.
...
~~At checkout, when a user subscribes to any tier, offer Auto Apply and/or Resume Builder as an order bump before the purchase completes — unchecked by default, cart total updates live if the user adds one, and it's dismissible without blocking the base subscription.~~ **Superseded 2026-09-02 — Auto Apply removed from this flow, see above.** The further unlocks inside each add-on (Full-Auto Mode, AI Suggestions) were never offered at this moment regardless — they were always meant to surface later, inside the add-on's own product experience, once the user is actually using it.
...
- **New subscription, with the add-on order bump.** *(Stale per the 2026-09-02 decision — §3.2 above. Auto Apply is no longer offered here; whether Resume Builder still is remains open.)* A user picks a tier and optionally adds Resume Builder before paying. One checkout, one payment, everything they picked included.
