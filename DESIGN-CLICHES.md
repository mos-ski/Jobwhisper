...
- **The sparkle/✨ icon as a stand-in for "AI."** The single most reflexive way to signal "this button does something AI" in every generated tool — instantly reads as templated, not designed. This product already has a real mark for that job: `JobwhisperAiIcon` (see `src/ui/brand-mark.tsx`, used by `AiSuggestionAction` and any "fill this from AI" affordance). Use it instead of `lucide-react`'s `Sparkles`, everywhere in this repo, no exceptions.
